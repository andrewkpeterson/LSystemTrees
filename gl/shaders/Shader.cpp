...
void Shader::setTexture(const std::string &name, const Texture2D &t, glm::vec2 repeatUV) {
    GLint location = m_textureLocations[name];
    GLint slot = m_textureSlots[location];
    glActiveTexture(GL_TEXTURE0 + slot);
    t.bind();
    glUniform1i(glGetUniformLocation(this->getID(), "useTexture"), 1);
    glUniform1i(location, slot);
    glUniform2fv(glGetUniformLocation(this->getID(), "repeatUV"), 1, glm::value_ptr(repeatUV));
}
